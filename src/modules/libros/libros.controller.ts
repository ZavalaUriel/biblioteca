...
export class LibrosController {
  ...
  async getLibros(
    ...
    const viewModelsLocales = librosLocales.map((l) =>
      LibroViewModel.fromEntity(l, 'interno'),
    );

    let viewModelsExternos: LibroViewModel[] = [];
    if (!isExternalRequest) {
      try {
        const librosExternos = await this.utlApiService.buscarLibrosExterno(
          query || '',
        );
        viewModelsExternos = librosExternos.map((l: any) =>
          LibroViewModel.fromEntity(l, 'externo'),
        );
      } catch (e) {
